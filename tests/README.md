...
Core tests take longer than any other Flurbo tests, due to the high amount of computational work involved in validating core components.
...
To run only Flurbo's core tests (after building):
...
To run only Flurbo's crypto tests (after building):
...
To run only Flurbo's hash tests (after building):
...
To run only Flurbo's performance tests (after building):
...
To run only Flurbo's unit tests (after building):
